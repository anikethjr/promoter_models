...
class MalinoisMPRADataset(Dataset):
    def __init__(self, df, split, num_cells, cell_names, \
        ...
        if self.use_cache and os.path.exists(os.path.join(self.cache_dir, f"{self.split}_seqs.npy")):
            self.all_seqs = np.load(os.path.join(self.cache_dir, f"{self.split}_seqs.npy"))
