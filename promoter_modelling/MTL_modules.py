...
class MTLPredictor(L.LightningModule):
    def __init__(self, \
        ...
        if self.use_simple_regression:
            ...
            assert self.all_dataloaders[0].name.startswith("Fluorescence") or self.all_dataloaders[0].name.startswith("Malinois"), "Simple regression only works with one dataloader and that can be FluorescenceData or Malinois_MPRA"
            ...
            for j, output in enumerate(self.all_dataloaders[i].output_names):
                if (not os.path.exists(os.path.join(cache_dir, "{}_{}_predictor.joblib".format(self.all_dataloaders[i].name, output)))) or (not use_existing_models):
                    ...
                    if self.all_dataloaders[i].name.startswith("Fluorescence"):
                        ...
                        y_test = self.y[test_inds, j][y_mask_test]

                        y_mask_val = self.y_mask[val_inds, j]
                        X_val = self.backbone_outputs[val_inds][y_mask_val]
                        y_val = self.y[val_inds, j][y_mask_val]
